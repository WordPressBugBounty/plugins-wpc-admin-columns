(function ($) {
    ...
    $(function () {
        ...
        if ($('.wpcac-actions').length && $('.tablenav').length) {
            ...
            if (wpcac_vars.intro === '' || wpcac_vars.intro === undefined) {
                $('.tablenav.top').find('.wpcac-btn').attr('data-intro', wpcac_vars.intro_text);

                let intro = introJs();

                intro.setOptions({'showBullets': false, 'doneLabel': wpcac_vars.intro_done}).oncomplete(function () {
                    $.post(ajaxurl, {action: 'wpcac_intro_done', nonce: wpcac_vars.nonce});
                });
            ...
            if (wpcac_vars.horizontal_scroll === 'yes') {
                ...
                $('.wp-list-table').wrap('<div class="wpcac-horizontal-scroll-table"></div>');
            }
    ...
    $(document).on('click touch', '.wpcac-copy', function (e) {
        ...
        let $this = $(this);
        let text = $this.closest('.wpcac-value-wrapper').find('.wpcac-value').text();
    ...
    $(document).on('click touch', '.wpcac-edit-save', function (e) {
        ...
        $.post(ajaxurl, data, function (response) {
            if (response.status) {
                ...
                if (parseInt(uid) > 0) {
                    $('.wpcac-value[data-uid="' + uid + '"][data-field="' + field + '"]').html(response.value);
                }

                if (parseInt(tid) > 0) {
                    $('.wpcac-value[data-tid="' + tid + '"][data-field="' + field + '"]').html(response.value);
                }
            } else {
                $('#wpcac-popup-edit').removeClass('wpcac-popup-loading');
    ...
    $(document).on('keyup change', '.wpcac_column_name', function (e) {
        $(this).closest('.wpcac-column').find('.wpcac-column-heading .name').text($(this).val());
    });

    $(document).on('change', '.wpcac_column_type', function (e) {
        $(this).closest('.wpcac-column').find('.wpcac-column-heading .type').text($(this).val());
        wpcac_init_type();
    });
    ...
    $(document).on('click touch', '.wpcac-save-btn', function (e) {
        var form_data = $('.wpcac-columns').find('input, select, button, textarea').serialize() || 0;
        var data = {
    ...
    $(document).on('click touch', '.wpcac-image-remove', function (e) {
        e.preventDefault();
        $(this).closest('.wpcac-image-selector').find('.wpcac-image-id').val('').trigger('change');
        $(this).closest('.wpcac-image-selector').find('.wpcac-image-preview').html('');
    });
    ...
    function wpcac_image_selector() {
        $('.wpcac-image-add').on('click touch', function (e) {
            ...
            if (wpcac_media.image_frame) {
                wpcac_media.image_frame.uploader.uploader.param('post_id', wpcac_media.upload_id);
            ...
            wpcac_media.image_frame.on('select', function () {
                ...
                var $preview = wpcac_media.button.closest('.wpcac-image-selector').find('.wpcac-image-preview');
                var $image_id = wpcac_media.button.closest('.wpcac-image-selector').find('.wpcac-image-id');
    ...
    function wpcac_images_selector() {
        $('.wpcac-images-add').on('click touch', function (e) {
            ...
            if (wpcac_media.images_frame) {
                wpcac_media.images_frame.uploader.uploader.param('post_id', wpcac_media.upload_id);
            ...
            wpcac_media.images_frame.on('select', function () {
                ...
                var $images = wpcac_media.button.closest('.wpcac-images-selector').find('ul.wpcac-images');
                ...
                selection.map(function (attachment) {
                    ...
                    if (attachment.id) {
                        var url = attachment.sizes.thumbnail ? attachment.sizes.thumbnail.url : attachment.url;
                        $images.append('<li data-id="' + attachment.id + '"><span href="#" class="wpcac-image-thumb"><a class="wpcac-image-remove" href="#"></a><img src="' + url + '" /></span></li>');
                    }
                });
    ...
    function wpcac_images_selector_remove() {
        $(document).on('click touch', '.wpcac-images-selector .wpcac-image-remove', function (e) {
            e.preventDefault();
    ...
    function wpcac_sortable() {
        $('.wpcac-columns').sortable({handle: '.move', placeholder: 'wpcac-column-placeholder'});
    }
    ...
    function wpcac_init_type() {
        $('.wpcac_column_type').each(function () {
            ...
            $(this).closest('.wpcac-column').find('.wpcac-hide-if-type-default').hide();
            $(this).closest('.wpcac-column').find('.wpcac-show-if-type-' + type).show();
        });
    }
    ...
    function wpcac_init_data_type() {
        $('.wpcac_data_type').each(function () {
            ...
            $(this).closest('.wpcac-column').find('.wpcac-hide-if-data-type-default').hide();
            $(this).closest('.wpcac-column').find('.wpcac-show-if-data-type-' + type).show();
        });
    }
